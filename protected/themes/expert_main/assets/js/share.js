...
(function ($) {
    $(function () {
        $('div.share42init').each(function (idx) {
            ...
            var l = '';
            for (j = 0; j < s.length; j++)l += '<span class="share42-item" style="display:inline-block;margin:0 0 0 6px;height:30px;"><a rel="nofollow" style="display:inline-block;width:30px;height:30px;margin:0;padding:0;outline:none;background:url(' + f + fn + ') -' + 30 * j + 'px 0 no-repeat" href=' + s[j] + ' target="_blank"></a></span>';
            el.html('<span id="share42"><span style="position: relative; top: -10px; margin-right: 5px;">Поделиться в соц. сетях</span>' + l + '</span>' + '' + '<style>.share42-item:hover > a {background-position-y:-30px !important;}</style>');
        })
    })
})(jQuery);