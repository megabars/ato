...
class MolpolitikImportCommand extends CConsoleCommand
{
    public $portalId = 288;

    public $executiveId = 43;
    ...
    public function actionIndex()
    {
        ...
        $links = array(
            'http://www.depms.ru/depms/documents.html' => 'Документ',
        );
        ...
        foreach ($links as $url => $type)
        {
            ...
            foreach ($html->find('.doc-name a') as $item)
            {
    ...
    {
        if (!$content = Yii::app()->curl->get('http://www.depms.ru' . $url))
            return null;
