		...
		if( $assignSelectOptions!==array() )
		{
		    ...
		    if( isset($_POST['AssignmentForm'])===true )
			{
				...
				if( $formModel->validate()===true )
				{
                    ...
                    if ($formModel->itemname == 'PortalAdmin')
                    {
                        if ($tmp = UsrAuthAssignment::model()->findByAttributes(array('itemname' => 'PortalAdmin', 'userid' => (string)$model->getId())))
                        {
                            $tmp->data = json_encode(array('portal_id' => (int)$formModel->data));
                            $tmp->save();
                        }
                    }
