...
$this->breadcrumbs=array(
    'общественные обсуждения нпа' => $this->createUrl('/discuss/back/index'),
...
<?php $this->widget('application.widgets.adminGridView', array(
	...
	'columns'=>array(
                array(
                    ...
                    'type' => 'raw',
                    'value' => function($data){
                        $model = Discuss::model()->resetScope()->findByPk($data->owner_id);
                        echo CHtml::link($model->title,
                            Yii::app()->controller->createUrl("/discuss/front/view", array("id" => $data->owner_id)),
                            array('target' => '_blank')
                        );
                    }
