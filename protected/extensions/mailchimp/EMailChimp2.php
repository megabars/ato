...
class EMailChimp2 extends MailChimpBase
{
    ...
    public function listGroups($counts=false)
    {
        $params = array(
            'id'       => $this->listId,
            'counts'      => $counts
    ...
    public function groupAdd($name='new group')
    {
        $params = array(
            'id'            => $this->listId,
            'name'    => $name,
        ...
        return $this->call('lists/interest-grouping-add', $params);
    }

    public function groupUpdateName($name='new group',$groupId)
    {
        $params = array(
            'grouping_id'   => $groupId,
            'name'    => 'name',
        ...
        return $this->call('lists/interest-grouping-update', $params);
    }
    ...
    public function groupDelete($groupId)
    {
        ...
        return $this->call('lists/interest-grouping-del', $params);
    }
    ...
    public function listMembers($status = self::STATUS_SUBSCRIBED, $since = null)
    {
        ...
        while(true)
        {
            ...
            $start++;
            if ($this->errorCode === false && (int)$retval['total'] > 0 && !empty($retval['data'])) {
                $result = array_merge($result, $retval['data']);
            }else
                break;
        }
